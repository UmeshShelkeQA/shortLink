...
if(isset($_POST['long_btn']))
{
	...
	if(isset($_COOKIE['current_url']))
	{
		...
		if(preg_match("/$customurl/",$checkurl[$size-1]))
			echo "<script>alert('URL ALREADY CREATED');</script>";	
		else{
			$short_url=get_custom_url(domain,$url,$customurl,$ssid);
			if($short_url==null)
				echo "<script>alert('URL NOT AVAILABLE.TRY ANOTHER URL');</script>";
			else
...
<head>
  ...
  <title>Create New CUSTOM Link</title>
...
<!--	<form action="" method="POST" onsubmit="return(checkField())">  -->
	...
	<input type="text" id="custom_url" name="custom_url" class="inp" placeholder="Enter Your Custom Link Name" ><br><br>
        ...
        <div class="inner-container" style="width: 50%;" >
            ...
            <div class="section-text align-center mbr-white mbr-fonts-style display-5" id="cpdiv">
				<?php
					if(!empty($short_url))
					{
						echo "<input type='text' readonly id='cptext' value='".domain."$short_url' style='align:center;width:100%;background:#df4125;border:none;text-align:center'>";
	...
	function checkField(){
		...
		if(checkurl==''||customurl==''){
			...
			if(window.XMLHttpRequest){
			  ...
			  var UrlToSend="jscustomurl.php?long_url="+checkurl+"&custom_url="+customurl;
			  var short_url='';
